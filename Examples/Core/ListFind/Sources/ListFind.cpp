...
namespace Examples {
  ...
  public:
    ...
    static void Main() {
      ...
      Book result = books.Find(pcf_delegate(const Book& book) {return book.iD.Equals(idToFind);});
      ...
      result = books.FindLast(pcf_delegate(const Book& book) {return book.publishDate < DateTime(2001, 1, 1);});
      if (result != Book::Empty())
      ...
      results = books.FindAll(pcf_delegate(const Book& book) {return book.price < 10;});
      if (results.Count != 0)
  ...
  private:
    static void FillList() {
      ...
      pcf_using (System::IO::StreamReader reader(System::IO::Path::Combine(System::IO::Path::GetTempPath(), "books.csv"))) {

...
namespace {
  ...
  TEST(Exception, GetStackTrace) {
    try { throw Exception(Caller("ExceptionUT.cpp", 43)); } catch (const Exception& e) { EXPECT_TRUE(e.ToString().Contains(" in ExceptionUT.cpp:43")); }
    try { throw ArgumentOutOfRangeException(Caller("ExceptionUT.cpp", 44)); } catch (const Exception& e) { EXPECT_TRUE(e.ToString().Contains(" in ExceptionUT.cpp:44")); }
  }
  
  TEST(Exception, GetMessage) {
    try { throw Exception(_caller); } catch (const Exception& e) { EXPECT_EQ( "Exception of type 'Switch::System::Exception' was thrown.", e.Message()); }
    try { throw SystemException(_caller); } catch (const Exception& e) { EXPECT_EQ(e.Message(), "System error."); }
    try { throw AccessViolationException(_caller); } catch (const Exception& e) { EXPECT_EQ(e.Message(), "Attempted to read or write protected memory. This is often an indication that other memory is corrupt."); }
    //try { throw AggregateException(_caller); } catch (const Exception& e) { EXPECT_EQ(e.Message(), "One or more errors occured."); }
    try { throw AppDomainUnloadedException(_caller); } catch (const Exception& e) { EXPECT_EQ(e.Message(), "Attempted to access an unloaded AppDomain."); }
    try { throw ApplicationException(_caller); } catch (const Exception& e) { EXPECT_EQ(e.Message(), "Error in the application."); }
    try { throw ArgumentException(_caller); } catch (const Exception& e) { EXPECT_EQ(e.Message(), "Value does not fall within the expected range."); }
    try { throw ArgumentNullException(_caller); } catch (const Exception& e) { EXPECT_EQ(e.Message(), "Value cannot be null."); }
    try { throw ArgumentOutOfRangeException(_caller); } catch (const Exception& e) { EXPECT_EQ(e.Message(),"Specified argument is out of range of valid values."); }
    try { throw ArithmeticException(_caller); } catch (const Exception& e) { EXPECT_EQ(e.Message(), "Overflow or underflow in the arithmetic operation."); }
    try { throw ArrayTypeMismatchException(_caller); } catch (const Exception& e) { EXPECT_EQ(e.Message(), "Attempt to access an element as a type incompatible with the array."); }
  
    try { throw BadImageFormatException(_caller); } catch (const Exception& e) { EXPECT_EQ(e.Message(), "Format for the executable or library is invalid."); }
    try { throw CannotUnloadAppDomainException(_caller); } catch (const Exception& e) { EXPECT_EQ(e.Message(), "Attempt to unload the AppDomain failed."); }
    try { throw ContextMarshalException(_caller); } catch (const Exception& e) { EXPECT_EQ(e.Message(), "Attempt to marshal an object across a context boundary."); }
    try { throw DataMisalignedException(_caller); } catch (const Exception& e) { EXPECT_EQ(e.Message(), "A datatype misalignment was detected in a load or store instruction."); }
    try { throw DivideByZeroException(_caller); } catch (const Exception& e) { EXPECT_EQ(e.Message(), "Attempt to divide by zero."); }
    try { throw DllNotFoundException(_caller); } catch (const Exception& e) { EXPECT_EQ(e.Message(), "Dll was not found."); }
    try { throw DuplicateWaitObjectException(_caller); } catch (const Exception& e) { EXPECT_EQ(e.Message(), "Duplicate object in argument."); }
    try { throw EntryPointNotFoundException(_caller); } catch (const Exception& e) { EXPECT_EQ(e.Message(), "Entry point was not found."); }
    try { throw ExecutionEngineException(_caller); } catch (const Exception& e) { EXPECT_EQ(e.Message(), "Internal error in the runtime."); }
    try { throw MemberAccessException(_caller); } catch (const Exception& e) { EXPECT_EQ(e.Message(), "Cannot acces member."); }
    try { throw FieldAccessException(_caller); } catch (const Exception& e) { EXPECT_EQ(e.Message(), "Attempted to access a field that is not accessible by the caller."); }
    try { throw FormatException(_caller); } catch (const Exception& e) { EXPECT_EQ(e.Message(), "One of identified items is an invalid format."); }
    try { throw IndexOutOfRangeException(_caller); } catch (const Exception& e) { EXPECT_EQ(e.Message(), "Index was outside the bounds of the array."); }
    try { throw InsufficientExecutionStackException(_caller); } catch (const Exception& e) { EXPECT_EQ(e.Message(), "Insufficient stack to continue executing the program safety. This can happen from having too many functions on the call stack using too much stack space."); }
    try { throw OutOfMemoryException(_caller); } catch (const Exception& e) { EXPECT_EQ(e.Message(), "Insufficient memory to continue the execution of the program."); }
    try { throw InsufficientMemoryException(_caller); } catch (const Exception& e) { EXPECT_EQ(e.Message(), "Insufficient memory to continue the execution of the program."); }
    try { throw InvalidCastException(_caller); } catch (const Exception& e) { EXPECT_EQ(e.Message(), "Specified cast is not valid."); }
    try { throw InvalidOperationException(_caller); } catch (const Exception& e) { EXPECT_EQ(e.Message(), "Operation is not valid due to the current state of the object."); }
    try { throw InvalidProgramException(_caller); } catch (const Exception& e) { EXPECT_EQ(e.Message(), "The system detected an invalid program."); }
    try { throw InvalidTimeZoneException(_caller); } catch (const Exception& e) { EXPECT_EQ(e.Message(), "The time zone information is not valid."); }
    try { throw MethodAccessException(_caller); } catch (const Exception& e) { EXPECT_EQ(e.Message(), "Attempt to acces the method failed."); }
    try { throw MissingMemberException(_caller); } catch (const Exception& e) { EXPECT_EQ(e.Message(), "Attempt to acces a missing member."); }
    try { throw MissingFieldException(_caller); } catch (const Exception& e) { EXPECT_EQ(e.Message(), "Attempt to acces a non-existing field."); }
    try { throw MissingMethodException(_caller); } catch (const Exception& e) { EXPECT_EQ(e.Message(), "Attempt to acces a missing method."); }
    try { throw MulticastNotSupportedException(_caller); } catch (const Exception& e) { EXPECT_EQ(e.Message(), "Attempt to add multiple callbacks to a delegate that does not support multicast."); }
    try { throw NotFiniteNumberException(_caller); } catch (const Exception& e) { EXPECT_EQ(e.Message(), "Number encountered was not a finite quantity."); }
    try { throw NotImplementedException(_caller); } catch (const Exception& e) { EXPECT_EQ(e.Message(), "The method or operation is not implemented."); }
    ...
    try { throw RankException(_caller); } catch (const Exception& e) { EXPECT_EQ(e.Message(), "Attempt to operate on an array with the incorrect number of dimensions."); }
    try { throw StackOverflowException(_caller); } catch (const Exception& e) { EXPECT_EQ(e.Message(), "Operation caused a stack overflow."); }
    try { throw TimeoutException(_caller); } catch (const Exception& e) { EXPECT_EQ(e.Message(), "The operation has timed out."); }
    try { throw TimeZoneNotFoundException(_caller); } catch (const Exception& e) { EXPECT_EQ(e.Message(), "The time zone cannot be found."); }
    try { throw TypeAccessException(_caller); } catch (const Exception& e) { EXPECT_EQ(e.Message(), "Attempt to access the type failed."); }
    ...
    try { throw IOException(_caller); } catch (const Exception& e) { EXPECT_EQ(e.Message(), "I/O error occured."); }
    try { throw DirectoryNotFoundException(_caller); } catch (const Exception& e) { EXPECT_EQ(e.Message(), "Attempted to access a path that is not on the disk."); }
    try { throw DriveNotFoundException(_caller); } catch (const Exception& e) { EXPECT_EQ(e.Message(), "Attempted to access a drive that is not avaible."); }
    try { throw EndOfStreamException(_caller); } catch (const Exception& e) { EXPECT_EQ(e.Message(), "Attempted to read past the end of the stream."); }
    ...
    try { throw ThreadStartException(_caller); } catch (const Exception& e) { EXPECT_EQ(e.Message(), "Thread start failure."); }
    try { throw ThreadStateException(_caller); } catch (const Exception& e) { EXPECT_EQ(e.Message(), "Thread was in an invalid state for the operation being executed."); }
    try { throw WaitHandleCannotBeOpenedException(_caller); } catch (const Exception& e) { EXPECT_EQ(e.Message(), "No handle of the given name exists."); }
  }

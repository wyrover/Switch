...
namespace {
  TEST(BitConverterTest, GetBytesBoolean) {
    using_(Array<byte> bytes = BitConverter::GetBytes(false)) {
      EXPECT_B1(0, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(true)) {
      EXPECT_B1(1, bytes);
    }
  }

  TEST(BitConverterTest, GetBytesChar) {
    using_(Array<byte> bytes = BitConverter::GetBytes(' ')) {
    ...
    using_(Array<byte> bytes = BitConverter::GetBytes('3')) {
      EXPECT_B4(0x33, 0x00, 0x00, 0x00, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes('A')) {
  ...
  TEST(BitConverterTest, GetBytesDouble) {
    using_(Array<byte> bytes = BitConverter::GetBytes(0.0)) {
      EXPECT_B8(0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(1.0)) {
      EXPECT_B8(0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xF0, 0x3F, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(255.0)) {
      EXPECT_B8(0x00, 0x00, 0x00, 0x00, 0x00, 0xE0, 0x6F, 0x40, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(4294967295.0)) {
      EXPECT_B8(0x00, 0x00, 0xE0, 0xFF, 0xFF, 0xFF, 0xEF, 0x41, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(0.00390625)) {
      EXPECT_B8(0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x70, 0x3F, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(0.00000000023283064365386962890625)) {
      EXPECT_B8(0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xF0, 0x3D, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(1.23456789012345E-300)) {
      EXPECT_B8(0xDF, 0x88, 0x1E, 0x1C, 0xFE, 0x74, 0xAA, 0x01, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(1.2345678901234565)) {
      EXPECT_B8(0xFA, 0x59, 0x8C, 0x42, 0xCA, 0xC0, 0xF3, 0x3F, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(1.2345678901234567)) {
      EXPECT_B8(0xFB, 0x59, 0x8C, 0x42, 0xCA, 0xC0, 0xF3, 0x3F, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(1.2345678901234569)) {
      EXPECT_B8(0xFC, 0x59, 0x8C, 0x42, 0xCA, 0xC0, 0xF3, 0x3F, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(1.23456789012345678E+300)) {
      EXPECT_B8(0x52, 0xD3, 0xBB, 0xBC, 0xE8, 0x7E, 0x3D, 0x7E, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(Double::MinValue)) {
      EXPECT_B8(0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xEF, 0xFF, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(Double::MaxValue)) {
      EXPECT_B8(0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xEF, 0x7F, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(Double::Epsilon)) {
      EXPECT_B8(0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(Double::NaN)) {
      EXPECT_B8(0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xF8, 0x7F, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(Double::NegativeInfinity)) {
      EXPECT_B8(0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xF0, 0xFF, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(Double::PositiveInfinity)) {
      EXPECT_B8(0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xF0, 0x7F, bytes);
    }
  }

  TEST(BitConverterTest, GetBytesInt16) {
    using_(Array<byte> bytes = BitConverter::GetBytes(int16(0))) {
      EXPECT_B2(0x00, 0x00, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(int16(15))) {
    ...
    using_(Array<byte> bytes = BitConverter::GetBytes(int16(10000))) {
    ...
    using_(Array<byte> bytes = BitConverter::GetBytes(Int16::MinValue)) {
      EXPECT_B2(0x00, 0x80, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(Int16::MaxValue)) {
      EXPECT_B2(0xFF, 0x7F, bytes);
    }
  }

  TEST(BitConverterTest, GetBytesInt32) {
    using_(Array<byte> bytes = BitConverter::GetBytes(0)) {
      EXPECT_B4(0x00, 0x00, 0x00, 0x00, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(15)) {
    ...
    using_(Array<byte> bytes = BitConverter::GetBytes(1048576)) {
    ...
    using_(Array<byte> bytes = BitConverter::GetBytes(1000000000)) {
    ...
    using_(Array<byte> bytes = BitConverter::GetBytes(Int32::MinValue)) {
      EXPECT_B4(0x00, 0x00, 0x00, 0x80, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(Int32::MaxValue)) {
      EXPECT_B4(0xFF, 0xFF, 0xFF, 0x7F, bytes);
    }
  }

  TEST(BitConverterTest, GetBytesInt64) {
    using_(Array<byte> bytes = BitConverter::GetBytes(0LL)) {
      EXPECT_B8(0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(0xFFFFFFLL)) {
      EXPECT_B8(0xFF, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x00, 0x00, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(1000000000LL)) {
    ...
    using_(Array<byte> bytes = BitConverter::GetBytes(0x100000000LL)) {
    ...
    using_(Array<byte> bytes = BitConverter::GetBytes(0xAAAAAAAAAAAALL)) {
    ...
    using_(Array<byte> bytes = BitConverter::GetBytes(1000000000000000000LL)) {
    ...
    using_(Array<byte> bytes = BitConverter::GetBytes(Int64::MinValue)) {
      EXPECT_B8(0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(Int64::MaxValue)) {
      EXPECT_B8(0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x7F, bytes);
    }
  }

  TEST(BitConverterTest, GetBytesSingle) {
    using_(Array<byte> bytes = BitConverter::GetBytes(0.0f)) {
      EXPECT_B4(0x00, 0x00, 0x00, 0x00, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(1.0f)) {
      EXPECT_B4(0x00, 0x00, 0x80, 0x3F, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(15.0f)) {
      EXPECT_B4(0x00, 0x00, 0x70, 0x41, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(65535.0f)) {
      EXPECT_B4(0x00, 0xFF, 0x7F, 0x47, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(0.00390625f)) {
      EXPECT_B4(0x00, 0x00, 0x80, 0x3B, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(0.00000000023283064365386962890625f)) {
      EXPECT_B4(0x00, 0x00, 0x80, 0x2F, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(1.2345E-35f)) {
      EXPECT_B4(0x49, 0x46, 0x83, 0x05, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(1.2345671f)) {
      EXPECT_B4(0x4B, 0x06, 0x9E, 0x3F, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(1.2345673f)) {
      EXPECT_B4(0x4D, 0x06, 0x9E, 0x3F, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(1.2345677f)) {
      EXPECT_B4(0x50, 0x06, 0x9E, 0x3F, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(1.23456789E+35f)) {
      EXPECT_B4(0x1E, 0x37, 0xBE, 0x79, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(Single::MinValue)) {
      EXPECT_B4(0xFF, 0xFF, 0x7F, 0xFF, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(Single::MaxValue)) {
      EXPECT_B4(0xFF, 0xFF, 0x7F, 0x7F, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(Single::Epsilon)) {
      EXPECT_B4(0x01, 0x00, 0x00, 0x00, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(Single::NaN)) {
      EXPECT_B4(0x00, 0x00, 0xC0, 0x7F, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(Single::NegativeInfinity)) {
      EXPECT_B4(0x00, 0x00, 0x80, 0xFF, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(Single::PositiveInfinity)) {
      EXPECT_B4(0x00, 0x00, 0x80, 0x7F, bytes);
    }
  }

  TEST(BitConverterTest, GetBytesUInt16) {
    using_(Array<byte> bytes = BitConverter::GetBytes(uint16(15))) {
      EXPECT_B2(0x0F, 0x00, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(uint16(1023))) {
      EXPECT_B2(0xFF, 0x03, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(uint16(10000))) {
      EXPECT_B2(0x10, 0x27, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(uint16(0))) {
      EXPECT_B2(0x00, 0x00, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(uint16(Int16::MaxValue))) {
      EXPECT_B2(0xFF, 0x7F, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(UInt16::MaxValue)) {
      EXPECT_B2(0xFF, 0xFF, bytes);
    }
  }

  TEST(BitConverterTest, GetBytesUInt32) {
    using_(Array<byte> bytes = BitConverter::GetBytes(15U)) {
      EXPECT_B4(0x0F, 0x00, 0x00, 0x00, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(1023U)) {
      EXPECT_B4(0xFF, 0x03, 0x00, 0x00, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(0x100000U)) {
      EXPECT_B4(0x00, 0x00, 0x10, 0x00, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(1000000000U)) {
      EXPECT_B4(0x00, 0xCA, 0x9A, 0x3B, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(UInt32::MinValue)) {
      EXPECT_B4(0x00, 0x00, 0x00, 0x00, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(uint32(Int32::MaxValue))) {
      EXPECT_B4(0xFF, 0xFF, 0xFF, 0x7F, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(UInt32::MaxValue)) {
      EXPECT_B4(0xFF, 0xFF, 0xFF, 0xFF, bytes);
    }
  }

  TEST(BitConverterTest, GetBytesUInt64) {
    using_(Array<byte> bytes = BitConverter::GetBytes(0xFFFFFFULL)) {
      EXPECT_B8(0xFF, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x00, 0x00, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(1000000000ULL)) {
      EXPECT_B8(0x00, 0xCA, 0x9A, 0x3B, 0x00, 0x00, 0x00, 0x00, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(0x100000000ULL)) {
      EXPECT_B8(0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(0xAAAAAAAAAAAAULL)) {
      EXPECT_B8(0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0x00, 0x00, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(1000000000000000000ULL)) {
      EXPECT_B8(0x00, 0x00, 0x64, 0xA7, 0xB3, 0xB6, 0xE0, 0x0D, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(10000000000000000000ULL)) {
      EXPECT_B8(0x00, 0x00, 0xE8, 0x89, 0x04, 0x23, 0xC7, 0x8A, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(UInt64::MinValue)) {
      EXPECT_B8(0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(uint64(Int64::MaxValue))) {
      EXPECT_B8(0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x7F, bytes);
    }

    using_(Array<byte> bytes = BitConverter::GetBytes(UInt64::MaxValue)) {

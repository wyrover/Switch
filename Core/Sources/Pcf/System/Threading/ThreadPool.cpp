...
RegisteredWaitHandle ThreadPool::RegisterWaitForSingleObject(WaitHandle& waitObject, const WaitOrTimerCallback& callBack, object& state, int32 millisecondsTimeoutInterval, bool executeOnlyOnce) {
  ...
  pcf_lock(threadPoolAsynchronousIOItems.SyncRoot) {
    ...
    SharedPointer<ThreadPoolAsynchronousIOItem> item = new ThreadPoolAsynchronousIOItem(callBack, state, waitObject, millisecondsTimeoutInterval, executeOnlyOnce);
...
void ThreadPool::Run() {
  while(!closed) {
    semaphore.WaitOne();
    if (!closed) {
      SharedPointer<ThreadPoolItem> item;
      pcf_lock(threadPoolItems.SyncRoot)
...
void ThreadPool::AsynchronousIORun() {
  while(!closed) {
    ...
    if (!closed) {
      SharedPointer<ThreadPoolAsynchronousIOItem> item;
      pcf_lock(threadPoolAsynchronousIOItems.SyncRoot)

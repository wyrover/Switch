...
refptr<Boolean> SerializationInfo::GetValue<Boolean>(const string& name) const {
  ...
  if (is<Boolean>(entry.Value().ToObject()))
    ...
    throw SerializationException(_current_information);
}

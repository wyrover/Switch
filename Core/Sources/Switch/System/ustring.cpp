...
namespace std {
  ...
  /*ustring::ustring(const char* str, int32_t startIndex, int32_t length) {
   if (str == null) throw System::ArgumentNullException(pcf_current_information);
   if ((size_t(length) != npos && length < 0) || startIndex < 0) throw System::ArgumentOutOfRangeException(pcf_current_information);

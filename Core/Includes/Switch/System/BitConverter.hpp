  ...
  namespace System {
    ...
    public:
      ...
      static String ToString(const Array<byte>& value, int32 startIndex, int32 length){
        ...
        if (startIndex < 0 || length < 0 || startIndex >= value.Length) throw ArgumentOutOfRangeException(_current_information);
        if (startIndex + length > value.Length) throw ArgumentException(_current_information);

...
namespace Switch {
  ...
  /// @remarks If you need to get the fully-qualified name, you can use the typeof expression along with nameof. For example:
  /// @code
  /// class C {
  /// public:
  ///   void f(int i) {
  ///     Log(string::Format("{0}.{1}", typeof(C), nameof(f)), "method entry");
  ///   }
  /// }
  /// @endcode
  /// @ingroup Switch
  #define nameof(value) System::String(#value)
}

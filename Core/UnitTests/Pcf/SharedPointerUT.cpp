...
namespace {
  ...
  TEST(Sp, SetNull) {
    pcf_using(SharedPointer<int> ptr) {
  ...
  TEST(Sp, SetNotNull) {
    ...
    pcf_using(SharedPointer<TestPointer> ptr(new TestPointer())) {
  ...
  TEST(Sp, Set) {
    ...
    EXPECT_EQ(1, TestPointer::GetCpt());
    pcf_using(SharedPointer<TestPointer> ptr(tp)) {
      EXPECT_NE((TestPointer*)null, ptr.ToPointer());
      EXPECT_FALSE(ptr.IsNull());
      EXPECT_NO_THROW(*ptr);
      EXPECT_NO_THROW(ptr.ToObject());
  ...
  TEST(Sp, SetFromSharedPointer) {
    TestPointer::ResetCpt();
    pcf_using(SharedPointer<TestPointer> ptr1(new TestPointer())) {
      EXPECT_EQ(1, TestPointer::GetCpt());
      pcf_using(SharedPointer<TestPointer> ptr2(ptr1)) {
        EXPECT_EQ(1, TestPointer::GetCpt());
  ...
  TEST(Sp, Constructor) {
    SharedPointer<string> stringEmpty;
    EXPECT_TRUE(stringEmpty.IsNull());
    EXPECT_TRUE(SharedPointer<string>::IsNullOrInvalid(stringEmpty));
    ...
    SharedPointer<string> stringEmpty2(SharedPointer<string>::Empty());
    EXPECT_TRUE(SharedPointer<string>::IsNullOrInvalid(stringEmpty2));
    ...
    SharedPointer<string> stringNull(null);
    EXPECT_TRUE(SharedPointer<string>::IsNullOrInvalid(stringNull));
    ...
    SharedPointer<string> str(new string("Test Share Pointer"));
    EXPECT_FALSE(SharedPointer<string>::IsNullOrInvalid(str));
    ...
    SharedPointer<string> str2= new string("Test an other Share Pointer");
    EXPECT_FALSE(SharedPointer<string>::IsNullOrInvalid(str2));
    ...
    EXPECT_EQ(18, str->Length());
    EXPECT_EQ('T', (*str)[0]);
    EXPECT_FALSE(SharedPointer<string>::IsNullOrInvalid(str3));
  ...
  TEST(Sp, Equal) {
    SharedPointer<string> stringEmpty = SharedPointer<string>::Empty();
    EXPECT_TRUE(SharedPointer<string>::IsNullOrInvalid(stringEmpty));
    EXPECT_THROW(stringEmpty.ToObject(), std::exception);
    EXPECT_THROW(stringEmpty.ToPointer(), std::exception);
    EXPECT_EQ(0, stringEmpty.GetUseCount());
    ...
    SharedPointer<string> stringNull = SharedPointer<String>::Null();
    EXPECT_TRUE(SharedPointer<string>::IsNullOrInvalid(stringNull));
    EXPECT_THROW(stringNull.ToObject(), std::exception);
    EXPECT_THROW(stringNull.ToPointer(), std::exception);
    EXPECT_EQ(0, stringNull.GetUseCount());
    ...
    *str3 = "And an other one";
    ...
    str2 = SharedPointer<string>::Null();
    EXPECT_FALSE(SharedPointer<string>::IsNullOrInvalid(str));
    EXPECT_EQ("And an other one", str.ToObject());
    EXPECT_EQ("And an other one", *str.ToPointer());
    EXPECT_EQ(1, str.GetUseCount());
    ...
    EXPECT_EQ(16, str->Length());
    EXPECT_EQ('A', (*str)[0]);
    EXPECT_TRUE(SharedPointer<string>::IsNullOrInvalid(str2));
    ...
    SharedPointer<string> str4 = str3;
    EXPECT_FALSE(SharedPointer<string>::IsNullOrInvalid(str3));
    EXPECT_EQ("Test an other Share Pointer", str3.ToObject());
    EXPECT_EQ("Test an other Share Pointer", *str3.ToPointer());
    EXPECT_EQ(2, str3.GetUseCount());
    ...
    EXPECT_EQ(27, str3->Length());
    EXPECT_EQ('T', (*str3)[0]);
    EXPECT_FALSE(SharedPointer<string>::IsNullOrInvalid(str4));
    ...
    str4 = SharedPointer<string>::Empty();
    EXPECT_FALSE(SharedPointer<string>::IsNullOrInvalid(str3));
    EXPECT_EQ("Test an other Share Pointer", str3.ToObject());
    EXPECT_EQ("Test an other Share Pointer", *str3.ToPointer());
    EXPECT_EQ(1, str3.GetUseCount());
    ...
    EXPECT_EQ(27, str3->Length());
    EXPECT_EQ('T', (*str3)[0]);
    EXPECT_TRUE(SharedPointer<string>::IsNullOrInvalid(str4));
    ...
    SharedPointer<string> str5 = str3;
    SharedPointer<string> str6 = str5;
    SharedPointer<string> str7 = str3;
    EXPECT_FALSE(SharedPointer<string>::IsNullOrInvalid(str3));
    EXPECT_EQ("Test an other Share Pointer", str3.ToObject());
    EXPECT_EQ("Test an other Share Pointer", *str3.ToPointer());
    ...
    EXPECT_EQ(27, str3->Length());
    EXPECT_EQ('T', (*str3)[0]);
    EXPECT_FALSE(SharedPointer<string>::IsNullOrInvalid(str5));
    ...
    EXPECT_EQ('T', (*str5)[0]);
    EXPECT_FALSE(SharedPointer<string>::IsNullOrInvalid(str6));
    ...
    EXPECT_EQ('T', (*str6)[0]);
    EXPECT_FALSE(SharedPointer<string>::IsNullOrInvalid(str7));
  ...
  TEST(Sp, Null) {
    /*
     SharedPointer<string> stringNull;
     ...
     SharedPointer<string> str =new string();
  ...
  TEST(Sp, Cast) {
    SharedPointer<string> str= new string("Test for cast");
    string strCopy;
  ...
  TEST(Sp, Release) {
    ...
    {
      SharedPointer<ParentA> parentA=new ParentA();
      SharedPointer<ParentA> parentA1=new ParentA();
  ...
  TEST(Sp, Copy) {
    ...
    {
      SharedPointer<ParentA> parentA=new ParentA();
      SharedPointer<ParentA> parentA1(parentA);
      EXPECT_EQ(0, ParentA::parentAReleaseCount);
    }
  ...
  TEST(Sp, Cast2) {
    ...
    {
      SharedPointer<ChildB> childB=new ChildB();
      SharedPointer<ParentA> parentA = childB.ChangeType<ParentA>();
      EXPECT_EQ(0, ParentA::parentAReleaseCount);
  ...
  TEST(Sp, ToObjectTT) {
    SharedPointer<string> spString(new string("Hello World"));
    SharedPointer<Object> obj = spString.ChangeType<Object>();
  ...
  TEST(Sp, Equality) {
    Pcf::SharedPointer<std::string> s1 = new std::string("string 1");
  }
  ...
  TEST(Sp, CopyCast) {
    ...
    {
      SharedPointer<ChildB> childB=new ChildB();
      //SharedPointer<ParentA> parentA(childB);
      SharedPointer<ParentA> parentA = childB.ChangeType<ParentA>();

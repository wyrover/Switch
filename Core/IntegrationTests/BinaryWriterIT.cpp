...
TEST(BinaryWriter, Ctor) {
  FileStream fs = File::Open("TestBinary.dat", FileMode::Create, FileAccess::Write);
  BinaryWriter writer(fs);
}

TEST(BinaryWriter, ReadWrite) {

  pcf_using(FileStream fs = File::Open("TestBinary.dat", FileMode::Create, FileAccess::Write)) {
    BinaryWriter binaryWriter(fs);
  ...
  pcf_using(FileStream fs = File::Open("TestBinary.dat", FileMode::Open, FileAccess::Read)) {
    BinaryReader binaryReader(fs);

...
// USA. // // Please report all bugs and problems to "fltk-bugs@easysw.com".
// 
...
public:
  Fl_Shaped_Window(int W, int H, const char *l = 0) : Fl_Window(W,H,l) {
    set_flag(8);
    shape_ = 0; 
    lw = lh = 0; 
    changed = 0; 
  ...
  Fl_Shaped_Window(int X, int Y, int W, int H, const char *l = 0) : Fl_Window(X,Y,W,H,l) {
    set_flag(8); 
    shape_ = 0; 
    lw = lh = 0; 
  } 
...
void Fl_Shaped_Window::draw() {
  if ((lw != w() || lh != h() || changed) && shape_) { 
    ...
    #ifdef _WIN32 HRGN region = bitmap2region(mask);
    ...
    #elif DJGPP 
    #else 
    Pixmap pmask = XCreateBitmapFromData(fl_display, fl_xid(this), (const char*)mask->array, mask->w(), mask->h());
    hide(); 
    XShapeCombineMask(fl_display, fl_xid(this), ShapeBounding, 0, 0, pmask, ShapeSet);
    show();
...
// maybe one day we'll want to be able to resize the clip mask 
// bitmap when the window is resized static 
Fl_Bitmap* resize_bitmap(Fl_Bitmap* bitmap, int /*W*/, int /*H*/) {

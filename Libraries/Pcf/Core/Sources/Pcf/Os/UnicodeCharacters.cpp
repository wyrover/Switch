...
namespace Pcf {
  namespace Os {
    bool UnicodeCharacters::ValidateCodePoint(uint32 codePoint) {
    ...
    bool UnicodeCharacters::IsAlpha(uint32 codePoint) {
    ...
    bool UnicodeCharacters::IsDigit(uint32 codePoint) {
      UnicodeAutomaton& a = GetUnicodeCharacters();
      UniquePointer<symbol[]> word = CodePointHexEncodingFunction(codePoint);
      return a.Get(word.ToPointer(),8) == UnicodeIsDigit;
    }
    
    bool UnicodeCharacters::IsPunctuation(uint32 codePoint) {
      UnicodeAutomaton& a = GetUnicodeCharacters();
      UniquePointer<symbol[]> word = CodePointHexEncodingFunction(codePoint);

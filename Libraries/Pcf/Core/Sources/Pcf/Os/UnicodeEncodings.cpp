...
byte UnicodeEncodings::UTF8::GetFormat(byte b) {
...
static bool readZeroFormats(const std::string& str, uint32 pos, byte nb) {
  for (uint32 i = pos+1; i < str.size() ; i += 1) {
    if (nb-- <= 0) return true;
    byte format = UnicodeEncodings::UTF8::GetFormat((byte)str[i]);
    if (format != 0)
        return false;
  }  
  return nb == 0;
...
int UnicodeEncodings::UTF8::GetNextCodeIndex(const std::string& str, int32 pos) {
  for (uint32 i = pos+1; i < str.size() ; i += 1) {
...
int UnicodeEncodings::UTF8::GetPrevCodeIndex(const std::string& str, int32 pos) {
...
uint32 UnicodeEncodings::UTF8::GetCode(const byte* bytes, byte size) {
  uint32 code = 0;
  byte offset = 0;
  byte rest = size - 1;
...
uint32 UnicodeEncodings::UTF8::GetLength(const std::string& str) {
  uint32 len = 0;
  for (int i = Os::UnicodeEncodings::UTF8::GetNextCodeIndex(str, -1); str.size() > 0 && i != -1; i = Os::UnicodeEncodings::UTF8::GetNextCodeIndex(str, i))
    len += 1;
  return len;
}

int UnicodeEncodings::UTF8::GetLastIndex(const std::string& str) {
  return GetPrevCodeIndex(str,static_cast<int32>(str.size()));
}

std::string UnicodeEncodings::UTF8::Encode(uint32 code) {
  std::string utf8_string;
  Encode(code,utf8_string);
  return utf8_string;
}

int UnicodeEncodings::UTF8::GetByteCount(uint32 code) {
...
int UnicodeEncodings::UTF8::Encode(uint32 code, byte bytes[]) {
  if (!UnicodeCharacters::ValidateCodePoint(code))
    return 0;
...
int UnicodeEncodings::UTF8::Encode(uint32 code, std::string& utf8_encoding) {
...
int UnicodeEncodings::UTF16::GetByteCount(uint32 code) {
  if (!UnicodeCharacters::ValidateCodePoint(code)) return 0;
...
int UnicodeEncodings::UTF16::Encode(uint32 code, byte bytes[], bool bigEndian ) {
  if (!UnicodeCharacters::ValidateCodePoint(code))
    return 0;
...
int UnicodeEncodings::UTF16::Decode(const byte* bytes, uint32 size , bool bigEndian, uint32& codePoint) {
  if (size < 2) return 0;
...
int UnicodeEncodings::UTF16::GetLength(const byte* bytes, uint32 nb_bytes, bool bigEndian) {
  MakeWord makeWordFunc = bigEndian ? &MakeWordBigEndian : &MakeWordLittleEndian;
...
void UnicodeEncodings::UTF32::Encode(uint32 code, byte& b1, byte& b2, byte& b3, byte& b4, bool bigEndian) {
  if (bigEndian) {
...
uint32 UnicodeEncodings::UTF32::Decode(byte b1, byte b2, byte b3, byte b4, bool bigEndian) {
  if (bigEndian) {
    return 
...
uint32 UnicodeEncodings::to_lower(uint32 c) {
  InitUnicode();
...
uint32 UnicodeEncodings::to_upper(uint32 c) {
  InitUnicode();
...
uint32 UnicodeEncodings::to_title(uint32 c) {
  InitUnicode();

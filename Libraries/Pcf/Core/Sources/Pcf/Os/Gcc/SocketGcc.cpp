...
namespace {
  ...
  static void swapsockaddr(byte* socketAddress) {
#if __APPLE__
    byte tmp = socketAddress[0];
    socketAddress[0] = socketAddress[1];
    socketAddress[1] = tmp;
    
    tmp = socketAddress[2];
    socketAddress[2] = socketAddress[3];
    socketAddress[3] = tmp;
...
int32 Socket::AddressFamilyToNative(System::Net::Sockets::AddressFamily addressFamily) {
#if __APPLE__
...
System::Net::Sockets::AddressFamily Socket::NativeToAddressFamily(int32 addressFamily) {
#if __APPLE_
...
int32 Socket::Accept(void* handle, byte* socketAddress, int32* addressLength, void** socket) {
  *socket = reinterpret_cast<void*>(accept(*(int32*)&handle, (struct sockaddr*)socketAddress, (socklen_t*)addressLength));
  if (*socket == (void*)-1)
    return -1;
  
  swapsockaddr(socketAddress);
  
  return 0;
}

int32 Socket::Bind(void* handle, byte* socketAddress, int32 addressLength) {
  return bind(*(int32*)&handle, (struct sockaddr*)socketAddress, (socklen_t)addressLength);
}

int32 Socket::Close(void* handle) {
  return close(*(int32*)&handle);
}

int32 Socket::Connect(void* handle, byte *socketAddress, int32 addressLength) {
  return ::connect(*(int32*)&handle, (struct sockaddr*)socketAddress, addressLength);
}

int32 Socket::GetAvailable(void* handle, int32* nbrBytesAvailable) {
  return ioctl(*(int32*)&handle, FIONREAD, nbrBytesAvailable);
}

int32 Socket::GetLastError() {
  return NativeToSocketError(errno);
}

bool Socket::GetOSSupportsIPv4() {
  return true;
}

bool Socket::GetOSSupportsIPv6() {
  return true;
}

int32 Socket::GetSocketOption(void* handle, System::Net::Sockets::SocketOptionLevel socketLevel, System::Net::Sockets::SocketOptionName socketOptionName, void* option, int32* optionLength) {
...
int32 Socket::IoCtl(void* handle, int32 ioControl, byte* optionInValue, int32 optionInValueSize, byte* optionOutValue, int32 optionOutValueSize, int32* optionOutValueSizeReturned) {
  return -1;
}

int32 Socket::Listen(void* handle, int32 backLog) {
  return listen(*(int32*)&handle, backLog);
}

int32 Socket::Open(System::Net::Sockets::AddressFamily addressFamily, System::Net::Sockets::SocketType socketType, System::Net::Sockets::ProtocolType protocolType, void** socket) {
  if (socket != null && (*socket = reinterpret_cast<void*>(::socket(AddressFamilyToNative(addressFamily), SocketTypeToNative(socketType), ProtocolTypeToNative(protocolType)))) != (void*)-1)
    return 0;
  
  return -1;
}

int32 Socket::Poll(void* handle, int32 microseconds, int32 mode) {
  if (handle == null || microseconds < 0 || mode < 0 || mode > 2)
    return -1;
...
int32 Socket::Select(void** checkRead, int32 nbCheckRead, void** checkWrite, int32 nbCheckWrite, void** checkError, int32 nbCheckError, int32 microseconds) {
  int nfds = 0;
...
int32 Socket::Receive(void* handle,  byte *buffer, int32 bufferLength, int32 flags) {
...
int32 Socket::ReceiveFrom(void* handle, byte* buffer, int32 bufferLength, int32 flags, byte *socketAddress, int32* addressLength) {
...
int32 Socket::Send(void* handle,  byte *buffer, int32 bufferLength, int32 flags) {
  return (int32)send(*(int32*)&handle, buffer, bufferLength, flags);
}

int32 Socket::SendTo(void* handle, byte* buffer, int32 bufferLength, int32 flags, byte *socketAddress, int32 addressLength) {
  return (int32)sendto(*(int32*)&handle, (char*)buffer, bufferLength, flags, reinterpret_cast<sockaddr*>(socketAddress), addressLength);
}

int32 Socket::SetBlocking(void* handle, bool blocking) {
  int32 retValue = -1;
...
int32 Socket::SetSocketOption(void* handle, System::Net::Sockets::SocketOptionLevel socketLevel, System::Net::Sockets::SocketOptionName socketOptionName, void* option, int32 optionLength) {
  if (socketLevel == System::Net::Sockets::SocketOptionLevel::Socket && (socketOptionName == System::Net::Sockets::SocketOptionName::SendTimeout || socketOptionName == System::Net::Sockets::SocketOptionName::ReceiveTimeout)) {
...
int32 Socket::Shutdown(void* handle, int32 how) {

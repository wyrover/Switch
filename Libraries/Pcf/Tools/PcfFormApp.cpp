  ...
  namespace System {
    namespace Windows {
      namespace Forms2 {
        ...
        using InvalidateEventHandler = TEventHandler<InvalidateEventArgs&>;
      }
    }
  }
}
    ...
    namespace Windows {
      namespace Forms2 {
        ...
        public:
          ...
          virtual void OnLocationChanged(const EventArgs& e) { this->LocationChanged(*this, e); }
          ...
          virtual void OnSizeChanged(const EventArgs& e) { this->SizeChanged(*this, e); }

          virtual void OnTextChanged(const EventArgs& e) { this->TextChanged(*this, e); }

          virtual void OnVisibleChanged(const EventArgs& e) { this->VisibleChanged(*this, e); }
        ...
        protected:
          ...
          virtual void CreateHandle() {OnHandleCreated(EventArgs());}
          
          virtual void DestroyHandle() {OnHandleDestroyed(EventArgs());}
          ...
          SharedPointer<ControlData> data = SharedPointer<ControlData>::Create();
          /// @endcond
        };
      }
    }
  ...
  namespace System {
    namespace Windows {
      namespace Forms2 {
        ...
        public:
          Form() {}
          Form(const Form& form) : Control(form) {}

          void OnInvalidated(InvalidateEventArgs& e) override {
            this->InvalidateControl(e);
            this->Control::OnInvalidated(e);
          }

          void OnLocationChanged(const EventArgs& e) override {
            this->ChangeLocation();
            this->Control::OnLocationChanged(e);
          }
          ...
          void OnTextChanged(const EventArgs& e) override {
            this->ChangeText();
            this->Control::OnTextChanged(e);
          }

          void OnVisibleChanged(const EventArgs& e) override {
            this->ChangeVisible();
            this->Control::OnVisibleChanged(e);
          }
        ...
        private:
          ...
          static LRESULT CALLBACK WindowProc(_In_ HWND hwnd, _In_ UINT uMsg, _In_ WPARAM wParam, _In_ LPARAM lParam) {
            ...
            System::Diagnostics::Debug::WriteLine("{0} Message = {1}", ++counter, MessageIdToString(uMsg), null);
            switch (uMsg) {
          ...
          void CreateControlHandle() {
            if (!this->data->handle) {
              ...
              if (!this->Parent().IsNull())
                this->data->handle = (intptr)CreateWindowExA(0, NULL, this->Text().Data, WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, (HWND)NULL, (HMENU)NULL, hInstance, (LPVOID)NULL);
              else {
                WNDCLASSA wc = {0, (WNDPROC)WindowProc, 0, 0, hInstance, LoadIcon((HINSTANCE)NULL, IDI_APPLICATION), LoadCursor((HINSTANCE)NULL, IDC_ARROW), (HBRUSH)COLOR_WINDOW, "MainMenu", "MainWndClass"};
                if (!RegisterClassA(&wc))
                  System::Diagnostics::Debug::WriteLine("RegisterCalssA error = {0}", GetLastError());
                this->data->handle = (intptr)CreateWindowExA(0, "MainWndClass", this->Text().Data, WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, (HWND)NULL, (HMENU)NULL, hInstance, (LPVOID)NULL);
              }
              if (!(HWND)this->data->handle)
                System::Diagnostics::Debug::WriteLine("CreateWindowA error = {0}", GetLastError());
              this->ChangeLocation();
              this->ChangeSize();
              this->ChangeVisible();
            }
          ...
          void DestroyControlHandle() {
            if (this->data->handle)
              DestroyWindow((HWND)this->data->handle);
            this->data->handle = 0;
          }

          void InvalidateControl(const InvalidateEventArgs& e) {
            if (this->data->handle)
              UpdateWindow((HWND)this->data->handle);
          }

          void ChangeLocation() {
            if (this->data->handle)
              SetWindowPos((HWND)this->data->handle, NULL, this->Location().X, this->Location().Y, this->Size().Width, this->Size().Height, SWP_NOSIZE);
          }

          void ChangeSize() {
            if (this->data->handle)
              SetWindowPos((HWND)this->data->handle, NULL, this->Location().X, this->Location().Y, this->Size().Width, this->Size().Height, SWP_NOMOVE);
          }

          void ChangeText() {
            if (this->data->handle)
              SetWindowTextA((HWND)this->data->handle, this->Text().Data());
          }

          void ChangeVisible() {
            if (this->data->handle)
              ShowWindow((HWND)this->data->handle, this->Visible ? SW_SHOW : SW_HIDE);
          }
          #else
          void CreateControl() {
          }
  ...
  namespace System {
    namespace Windows {
      namespace Forms2 {
        ...
        public:
          Button() {}
          Button(const Button& button) : Control(button) {}
    ...
    namespace Windows {
      namespace Forms2 {
        ...
        public:
          ...
          static void Run(const TForm& form) {
            mainForm = form.MemberwiseClone<TForm>().As<Form>();
            Run();
          }
        ...
        private:
          #if _WIN32
          static void SetWindowTheme() {
            if (!mainForm.IsNull())
              ::SetWindowTheme((HWND)mainForm().Handle(), NULL, NULL);
          }
...
using namespace System::Windows::Forms2;

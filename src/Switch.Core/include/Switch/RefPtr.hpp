/// @file
/// @brief Contains Switch::_ <T> class.
#pragma once
...
namespace Switch {
  ...
  /// In this figure, there are two _ objects pointing to the same sub-object (which is actually part of the _ structure hidden from the user), which then points to the data object which is the user's data. The alias count is stored in the sub-object. There is an alias count of 2 because there are two _ objects pointing to the sub-object.
  ...
  /// @par Examples
  /// This example show how to used _ :
  /// @include RefPtr.cpp
  template<typename T>
  class _ {
  public:
  ...
  /// @remarks In fact, an extra level of indirection needs to be used. A Reference Pointer class in fact contains a pointer to a sub-object which in turn contains the address of the contained object and an integer containing the alias count.
  /// @image html sharepointer.gif
  /// @remarks In this figure, there are two _ objects pointing to the same sub-object (which is actually part of the _ structure hidden from the user), which then points to the data object which is the user's data. The alias count is stored in the sub-object. There is an alias count of 2 because there are two _ objects pointing to the sub-object.

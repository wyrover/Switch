...
void Native::CoreApi::Dns::Destroy(intptr host) {
  delete (Hostent*)host;
}

void Native::CoreApi::Dns::EndHostent() {
  endhostent();
}

intptr Native::CoreApi::Dns::GetHostByAddress(const string& hostAddress, System::Net::Sockets::AddressFamily hostAddressType) {
...
intptr Native::CoreApi::Dns::GetHostByName(const string& hostName) {
  std::lock_guard<std::mutex> lock(mutex);
...
System::Array<string> Native::CoreApi::Dns::GetAliases(intptr host) {
  int32 index = 0;
...
System::Array<System::Net::IPAddress> Native::CoreApi::Dns::GetAddresses(intptr host) {
  int32 index = 0;
...
string Native::CoreApi::Dns::GetHostName(intptr host) {
  return ((Hostent*)host)->h_name;
}

int Native::CoreApi::Dns::GetHostName(string& hostName) {
  char host[1024];
...
void Native::CoreApi::Dns::SetHostent(bool stayOpen) {
  sethostent(stayOpen);
}

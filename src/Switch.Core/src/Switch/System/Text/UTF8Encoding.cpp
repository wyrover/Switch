...
int32 UTF8Encoding::Encoder::GetNbBytes(char32 c) const {
  return Native::CoreApi::UnicodeEncodings::UTF8::GetByteCount(c);
}

void UTF8Encoding::Encoder::Encode(char32 c, byte bytes[]) const {
  Native::CoreApi::UnicodeEncodings::UTF8::Encode(c, bytes);
}
...
void UTF8Encoding::Decoder::Add(byte b) {
  ...
  if (finished || (count > 0 && Native::CoreApi::UnicodeEncodings::UTF8::GetFormat(b) != 0))
    Reset();
  Encoding::Decoder::Add(b);
  if (count == 1) {
    format = Native::CoreApi::UnicodeEncodings::UTF8::GetFormat(b);
  ...
  if (format == count) {
    codePoint = Native::CoreApi::UnicodeEncodings::UTF8::GetCode(bytes,static_cast<byte>(count));
    finished = true;
  } else {
    if (count == 4) Reset();
  }
...
int32 UTF8Encoding::GetCharCount(const byte bytes[], int32 bytesSize, int32 index, int32 count) const {
  ...
  return Native::CoreApi::UnicodeEncodings::UTF8::GetLength(s);
}
